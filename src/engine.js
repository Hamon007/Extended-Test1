...
export function createInitialState() {
  return {
    ...
    flags: {
      ...
      seraVaterWahrheit: false,       // Sera kennt Wahrheit über Vater
      gregorVerletzt: false,          // Gregor verletzt im Kampf
      gregorWeissAlles: false,        // Gregor kennt Kaitos wahre Natur
      vexGemerk: false,               // Inquisitor Vex hat Kaito bemerkt
      ...
      renBond: false,                 // Bond mit Geist Ren aufgebaut
      veyraZugang: false,             // Veyra-Zugang erhalten
      ariaZugang: false,              // Aria-Zugang erhalten
      ...
      rolandFluchWissen: false,       // Kaito weiß von Rolands Fluch
      ...
      voidMemoryModel: false,         // Void-Gedächtnis-Modell aktiviert
...
// |--------|--------------------|-------|-----------------------------------------|
// |   1    | Der neue Gott      |  E    | licht > schatten UND ordnung > chaos                        |
// |   2    | Im Verborgenen |  B    | schatten >= licht                       |
...
// |   6    | Rueckkehr          |  D    | yukiGeheilt===true (Node-requires) |
// |   7    | System Delete      |  C    | immer     |
//
export function calculateEnding(state, pendingChoice = null) {
  const alignment = { ...state.alignment };
  const flags = { ...state.flags };

  // If ending is calculated directly from a final choice, simulate that choice
  // first so finaleWahl and alignment deltas are included.
  if (pendingChoice?.alignEffect) {
    for (const [key, val] of Object.entries(pendingChoice.alignEffect)) {
      const delta = getScaledAlignmentDelta(key, val);
      alignment[key] = (alignment[key] || 0) + delta;
    }
  }
  ...
  // Ending 1: New God
  if (w === "E") {
    return (alignment.licht > alignment.schatten && alignment.ordnung > alignment.chaos)
      ? "ending_1"
      : "ending_4";
  }
  ...
  // Ending 7: System Delete (always)
  if (w === "C") return "ending_7";

  // Ending 6: Return (node-level requires handles yukiGeheilt lock)
...
function getScaledAlignmentDelta(key, value) {
